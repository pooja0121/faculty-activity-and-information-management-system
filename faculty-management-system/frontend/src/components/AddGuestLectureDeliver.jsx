...
import './AddGuestLectureDeliver.css';

const AddGuestLectureDeliver = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        alert('Guest lecture deliver entry saved successfully');
        navigate('/faculty-dashboard');
      } else {
        alert('Failed to save guest lecture deliver entry');
      }
    } catch (error) {
      alert('Error saving guest lecture deliver entry');
    }
  };
  ...
  return (
    <div className="add-guest-lecture-deliver-container">
      <button className="back-button" onClick={handleCancel}>← Back to Dashboard</button>
      <h1>Add Guest Lecture Deliver</h1>
      <p>Fill in the details for your guest lecture deliver entry.</p>
...
export default AddGuestLectureDeliver;
